...
create table if not exists bookings (id uuid primary key default gen_random_uuid(), customer_id uuid references customers(id), booking_date date not null, start_time time not null, end_time time not null, service_type text not null check (service_type in ('hair','makeup','both')), service_detail text, requested_staff_id uuid references staff(id), assigned_staff jsonb, staff_sequence jsonb, customer_memo text, admin_memo text, status text default 'pending' check (status in ('pending','confirmed','rejected','cancelled')), rejection_reason text, alternative_date date, alternative_time time, confirmed_at timestamptz, rejected_at timestamptz, cancelled_at timestamptz, created_at timestamptz default now());
